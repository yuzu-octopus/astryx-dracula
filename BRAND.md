...
Source of truth is `astryx-theme.ts`. `tokens.json` records primitives, dims, and core semantic tokens; chart ramps live in `scripts/generate-chart-ramps.ts`. Do not add hexes; `bun scripts/audit-dracula.ts` enforces the palette.
...
- Motion durations: pinned to neutral defaults (fast 175ms, medium 410ms) instead of Stone's snappier curve.
- Spacing/size/shadow/ease scales: no brand opinion; Stone defaults read fine on dark.
