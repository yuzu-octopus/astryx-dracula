...
Import `astryx-dracula/tokens.css` for plain CSS vars (full mirror, 289 `:root` vars), or
`astryx-dracula` (`astryxDraculaTheme`) + `astryx-dracula/theme.css`
with `<Theme theme mode="dark">` from `@astryxdesign/core/theme` for the full 200-key surface.
